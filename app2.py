...
# ---------------------------------
# Callbacks: keep Search box and Select-Food dropdown mutually exclusive
# ---------------------------------
def clear_food_box():
    # Search madhun typing zali tar dropdown selection clear kara
    if st.session_state.get("search_box"):
        if "food_box" in st.session_state:
            del st.session_state["food_box"]

def clear_search_box():
    # Dropdown madhun food select zala tar search clear kara
    if st.session_state.get("food_box"):
...
if search.strip():
    ...
    if len(search_items) == 0:
        ...
        st.warning("⚠️ No matching food item found.")

    elif len(search_items) == 1:

        search_selected = search_items[0]
...
if len(filtered_items) == 0:
    ...
    st.info("🍽️ No matching food items found. Please search your food using the search bar above.")
    ...
    selected = st.selectbox(
        ...
        index=None,
        key="food_box",
        placeholder="Select food from filtered menu...",
